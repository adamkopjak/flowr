...
// ---------- Sparkline ----------
function Sparkline({ data, width=120, height=36, positive=true, animate=true, fill=true, gradientId, responsive=false }){
  if(!data || data.length < 2) return <div style={{width: responsive?'100%':width, height}} className="skeleton"/>;
  const pts = data;
  ...
  return (
    <svg width={responsive?'100%':width} height={height} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" style={{display:'block'}}>
      <defs>
  ...
  return (
    ...
    >
      ...
      <div style={{marginTop:'auto', marginLeft:-4, marginRight:-4}}>
        <Sparkline data={coin.sparkline_in_7d?.price} width={300} height={56} positive={pos} responsive/>
      </div>
    </button>
  );
}
  ...
  return (
    <div className="card fade-up" style={{padding:20, display:'flex', flexDirection:'column', gap:14}}>
      ...
      <div style={{margin:'0 -8px'}}>
        <Sparkline data={points} width={320} height={86} positive={pos} responsive/>
      </div>
...
function PriceRow({ coin, idx, isWatched, onWatch, onSelect }){
  const pos = (coin.price_change_percentage_24h ?? 0) >= 0;
  return (
    <div className="row price-grid" style={{ borderRadius: 18, cursor:'pointer' }} onClick={()=>onSelect && onSelect(coin)}>
      <button onClick={(e)=>{ e.stopPropagation(); onWatch(coin.id); }} className="col-watch" style={{
        background:'transparent', border:'none', cursor:'pointer',
        color: isWatched ? 'var(--accent)' : 'var(--text-faint)',
        fontSize: 16, padding: 0, lineHeight: 1
      }} aria-label="toggle watchlist">{isWatched?'★':'☆'}</button>
      <div className="mono col-rank" style={{color:'var(--text-faint)', fontSize:12}}>{String(idx+1).padStart(2,'0')}</div>
      <div className="col-asset" style={{display:'flex', alignItems:'center', gap:12, minWidth:0}}>
      ...
      <div className="mono col-price" style={{fontSize:14, fontWeight:500, textAlign:'right'}}>${fmtPrice(coin.current_price)}</div>
      <div className="col-change" style={{textAlign:'right'}}>
        <span className={pos?'chip chip-pos':'chip chip-neg'}>{fmtPct(coin.price_change_percentage_24h)}</span>
      </div>
      <div className="mono col-mcap" style={{fontSize:13, color:'var(--text-dim)', textAlign:'right'}}>${fmtBig(coin.market_cap)}</div>
